...
The starting point was ~2560 ms. The fast path is what an ordinary sentence takes: spoken long enough that partials arrive before it ends, so the final is skipped and most of the request has already flown. A one-word utterance cannot take it — nothing has been transcribed yet when the silence runs out — and pays the slow path. The hangover is now ~63 % of what remains, and it is the only link no shortcut has yet touched.
...
- **Endpointing is two-stage, and driven by evidence rather than by the clock.** Silence is weak evidence that a person finished talking — strong enough only after a long wait, which is why the hangover sits at the front of every reply. So the VAD also emits an early *pause hint* at 280 ms, and the orchestrator answers it with evidence the VAD doesn't have: whether the transcript just closed a sentence. Voxtral punctuates, and a full stop is the one signal that distinguishes finishing a thought from drawing breath. A hint over a transcript that stops mid-clause is ignored and the full hangover runs. This matters because the hangover is not padding: an utterance split in two loses its second half, which arrives while the mic is gated behind the readback of the first.
  ...
  What the hint actually buys, measured, is not an earlier ending. On device the turn still ends on silence (`endpoint: "silence"`): the settled partial has not closed with a full stop by the time it settles, because punctuating the tail is part of what the server's final tidies. The two-stage endpoint pays for itself through the *other* two things the hint arms — the speculative translation and the skipped final — and the hangover remains the largest single link. Ending the turn on the transcript rather than on silence is still the open prize.
